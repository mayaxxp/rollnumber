<template>
  <div id="secendpage">
    ...
    <component :is="getcurrRightPage" :msg="getcurrRightPage"></component>
  </div>
</template>
...
export default {
  name: 'Firstpage',
  components: { MainPageTop: MainPageTop,
    LeftMenu: LeftMenu,
    HelloWorld,
  ...
  mounted () {
			if(window.sessionStorage.getItem('jsnum')){
				...
				console.log(window.sessionStorage.getItem('jsnum'))
			}
  },
  computed: {
    getinitData () {
    ...
    getcurrRightPage () {
      var currentpagedata = this.$store.state.centermainpage;
      this.currentLink = currentpagedata.topageDemo
      return (currentpagedata.topageDemo)
    }
  },
  watch: {
