...
export function hmn_panel(id, type, w, h, titleName, data, contentDiv) {
	let obj = new Object();
	obj.id = id;
	obj.type = type;
	obj.w = w;
	...
	obj.bindData = function(data_) {
		window.document.getElementById("#" + id).querySelector(".Hmn_panelNormal_top_center")[0].html(data);
	}
	obj.resize = function(w_, h_) {
		window.document.getElementById("#" + id).querySelector(".Hmn_panelNormal_top_center")[0].style.cssText = '"width":' + (parseInt(w_) - 30) + '"px"';
		window.document.getElementById("#" + id).querySelector(".Hmn_panelNormal_Mid_center")[0].style.cssText = '"width":' + (parseInt(w_) - 30) + '"px"';
		window.document.getElementById("#" + id).querySelector(".Hmn_panelNormal_Bottom_center")[0].style.cssText = '"width":' + (parseInt(w_) - 30) + '"px"';
		window.document.getElementById("#" + id).querySelector(".Hmn_panelNormal_mid")[0].style.cssText = '"height":' + (parseInt(h_) - 52) + '"px"';
	...
	if(obj.type == "Hmn_panel_Nar") {
		...
		obj.resize = function(w_, h_) {
			...
			window.document.getElementById("#" + id)[0].style.cssText = '"width":' + (w_) + '"px";"height":' + (h_) + '"px"';
			window.document.getElementById("#" + id).querySelector(".Hmn_panelNormal_top_center")[0].style.cssText = '"width":' + (parseInt(w_) - 30) + '"px"';
			window.document.getElementById("#" + id).querySelector(".Hmn_panelNormal_Mid_center")[0].style.cssText = '"width":' + (parseInt(w_) - 30) + '"px"';
			window.document.getElementById("#" + id).querySelector(".Hmn_panelNormal_Bottom_center")[0].style.cssText = '"width":' + (parseInt(w_) - 30) + '"px"';
			window.document.getElementById("#" + id).querySelector(".Hmn_panelNormal_mid")[0].style.cssText = '"height":' + (parseInt(h_) - 52) + '"px"';
	...
	if(obj.type == "Hmn_panel_AC") {
		/*20181129调整后*/
		obj.dom = '<div id="' + obj.id + '"  class="Hmn_panel_content" style="width:' + (parseInt(obj.w) + 75) + 'px;height:' + (parseInt(obj.h) + 63) + 'px;opacity:1; top:' + ((parseInt(obj.h) + 63) / -2) + 'px;left:' + ((parseInt(obj.w) + 75) / -2) + 'px;position: absolute;">' +
		...
		obj.resize = function(w_, h_) {
			window.document.getElementById("#" + id)[0].style.cssText = '"width":' + (w_ + 75) + '"px";"height":' + (h_ + 63) + '"px"';
			window.document.getElementById("#" + id).querySelector(".Hmn_panelAC_top_center")[0].style.cssText = '"width":' + (w_ - 120 + 75) + '"px"';
			window.document.getElementById("#" + id).querySelector(".Hmn_panelAC_Mid_center")[0].style.cssText = '"width":' + (w_ - 120 + 75) + '"px"';
			window.document.getElementById("#" + id).querySelector(".Hmn_panelAC_Bottom_center")[0].style.cssText = '"width":' + (w_ - 120 + 75) + '"px"';
			window.document.getElementById("#" + id).querySelector(".Hmn_panelAC_mid")[0].style.cssText = '"height":' + (h_ - 140 + 63) + '"px"';
			//       
		};
	}
	if(obj.type == "Hmn_panel_BC") {
		...
		obj.resize = function(w_, h_) {
			...
			$("#" + id).find(".Hmn_panelBC_Mid_center").css({
				"width": (parseInt(w_) - 43) + "px"
			});
			$("#" + id).find(".Hmn_panelBC_Bottom_center").css({
				"width": (parseInt(w_) - 43) + "px"
			});
			$("#" + id).find(".Hmn_panelBC_mid").css({
				"height": (parseInt(h_) - 60) + "px"
			});

			window.document.getElementById("#" + id)[0].style.cssText = '"width":' + (w_) + '"px";"height":' + (h_) + '"px"';
			let serWidth = window.document.getElementById("#" + id).querySelector(".Hmn_panelBC_top_repeat1")[0].offsetWidth
			window.document.getElementById("#" + id).querySelector(".Hmn_panelBC_top_repeat2")[0].style.cssText = '"width":' + (parseInt(obj.w) - 43 - 98 - serWidth) + '"px"';
			window.document.getElementById("#" + id).querySelector(".Hmn_panelBC_Mid_center")[0].style.cssText = '"width":' + (parseInt(w_) - 43) + '"px"';
			window.document.getElementById("#" + id).querySelector(".Hmn_panelBC_Bottom_center")[0].style.cssText = '"width":' + (parseInt(w_) - 43) + '"px"';
			window.document.getElementById("#" + id).querySelector(".Hmn_panelBC_mid")[0].style.cssText = '"height":' + (parseInt(h_) - 60) + '"px"';
	...
	obj.colseWindow = function() {
		...
		setTimeout(function() {
			contentDiv.removeChild(window.document.getElementById("#" + id))
			
		}, 500);
		window.document.querySelector(".dataview_popwindow")[0].classList.add("hidd_popwindow");
		contentDiv.removeChild();
	};
	//contentDiv.appendChild(obj.dom);
//	 let kj=new DOMParser().parseFromString(obj.dom,'text/html')
//		(window.document.body).appendChild(kj);
	 var doma=document.createElement('div');
		doma.className='topcontentBox';
		doma.innerHTML=obj.dom;
		document.body.appendChild(doma);
	/*20181129调整后*/
	//点击关闭窗口事件
	window.document.getElementById("#" + id).querySelector(".Hmn_panelAC_closeBtn")[0].onclick=function() {
//		$("#" + id).animate({
//			"opacity": 0.0
//		}, 500)
		setTimeout(function() {
		...
		window.document.querySelector(".dataview_popwindow")[0].classList.add("hidd_popwindow");
//		let kj=new DOMParser().parseFromString(obj.dom,'text/html').body.childNodes[0]
//		(window.document.body).appendChild(kj);
	};
...
//		let pleft = parseInt(obj.domleft) + (obj.mouseNewPos.x - obj.mousePos.x);
//		let pbottom = parseInt(obj.dombottom) + (obj.mousePos.y - obj.mouseNewPos.y);
//		$(".Hmn_panel_content").css({
//			"left": pleft + "px"
//		})
//		$(".Hmn_panel_content").css({
//			"bottom": pbottom + "px"
...
//	}
	/*弹出框拖动相关end*/
	let sleW=window.document.getElementById("#" + id).querySelector(".Hmn_panelBC_top_repeat1")[0].offsetWidth
	window.document.getElementById("#" + id).querySelector(".Hmn_panelBC_top_repeat2")[0].style.cssText = '"width":' + ((parseInt(obj.w) - 43 - 98) - sleW) + '"px"';
 ...
 * @param: contentDiv 为父容器对象;
 **/
export function Hmn_noClosepanel(id, type, w, h, titleName, data, contentDiv) {
	let obj = new Object();
	obj.id = id;
	obj.type = type;
	obj.w = w;
